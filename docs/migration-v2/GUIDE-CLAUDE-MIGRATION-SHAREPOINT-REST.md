...
| 9 — Documents | ✅ Fait | `src/utils/documentStore.js` (validation, dossiers, `Files/add`, index `CN_FilesIndex`) ; les pièces jointes des annotations (`App.jsx`) et des commentaires compliance (`SynthesisReport.jsx`) ne sont plus stockées en base64. Depuis le 03/09/2026, l'URL de la bibliothèque `CN-Documents` (et `CN-Config`, §7) n'est plus reconstruite depuis le nom configuré mais résolue via `src/utils/spLibraryUrl.js` — voir le constat du 03/09/2026 dans `VERIFICATION-CONFIGURATION-SHAREPOINT.md` |
