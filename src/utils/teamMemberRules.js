...
// (« sauf si… »). Un membre sans critère est toujours déclenché — c'est le comportement
// historique, et c'est aussi ce qui garantit qu'au moins une personne est sollicitée quelles
// que soient les réponses (cf. hasUnconditionalTeamMember / getTeamMemberCoverageWarning).
export const MEMBER_TRIGGER_MODE_INCLUDE = 'include';
export const MEMBER_TRIGGER_MODE_EXCLUDE = 'exclude';

export const MEMBER_TRIGGER_MODES = [MEMBER_TRIGGER_MODE_INCLUDE, MEMBER_TRIGGER_MODE_EXCLUDE];

const normalizeMode = (mode) =>
  mode === MEMBER_TRIGGER_MODE_EXCLUDE ? MEMBER_TRIGGER_MODE_EXCLUDE : MEMBER_TRIGGER_MODE_INCLUDE;
...
export const normalizeTeamMemberRules = (team) => {
  ...
  return rawRules
    ...
    .filter((entry) => {
      ...
      seen.add(key);
      return hasMemberConditions(entry);
    });
};
...
export const isTeamMemberTriggered = (rule, answers = {}) => {
  if (!rule || !hasMemberConditions(rule)) {
    return true;
  }
...
export const applyTeamMemberRule = (team, email, updater) => {
  ...
  const memberRules = hasMemberConditions(next)
