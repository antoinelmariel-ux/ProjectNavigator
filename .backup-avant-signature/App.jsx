...
const updateProjectFilters = useCallback((updater) => {
    ...
    const hydrateFromSharePoint = async () => {
      try {
        ...
        if (referentials.missing.length > 0) {
          warnings.push(
            `configuration non publiée (${referentials.missing.join(', ')})`
          );
        }
        if (referentials.errors.length > 0) {
          warnings.push(
            `fichiers illisibles : ${referentials.errors.map((entry) => entry.file).join(', ')}`
          );
        }
        ...
        setSharePointSync({
          ...
          message: expired ? '' : (error && error.message) || 'Erreur inconnue.'
        });
      }
    };
  ...
  const handleAddAnnotationNote = useCallback((clientX, clientY, targetElement = null) => {
    ...
    showcaseStickyNotesProvider.upsertNote(newNote, { userEmail: currentUserEmail }).catch((error) => {
      if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn('[Post-its] Création SharePoint impossible :', error);
      }
    });
  }, [
  ...
  const handleAnnotationTextChange = useCallback((noteId, text) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (note) {
        showcaseStickyNotesProvider.upsertNote(note, { userEmail: currentUserEmail }).catch((error) => {
          if (typeof console !== 'undefined' && typeof console.warn === 'function') {
            console.warn('[Post-its] Sauvegarde SharePoint impossible :', error);
          }
        });
      }
  ...
  const handleCloseAnnotationNote = useCallback((noteId) => {
    ...
    showcaseStickyNotesProvider.upsertNote(closedNote, { userEmail: currentUserEmail }).catch((error) => {
      if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn('[Post-its] Clôture SharePoint impossible :', error);
      }
    });
  ...
  const handleAddAnnotationReply = useCallback((noteId, replyText) => {
    ...
    showcaseStickyNotesProvider.upsertNote(updatedNote, { userEmail: currentUserEmail }).catch((error) => {
      if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn('[Post-its] Réponse SharePoint impossible :', error);
      }
    });
      ...
      if (project && nextComments && typeof nextComments === 'object' && !Array.isArray(nextComments)) {
        ...
        const mirrorChangedEntries = (entries, previousEntries, targetType) => {
          Object.entries(entries).forEach(([targetId, entry]) => {
            ...
            complianceCommentsProvider
              .upsertComment(activeProjectId, targetType, targetId, entry, { userEmail: currentUserEmail })
              .catch((error) => {
                if (typeof console !== 'undefined' && typeof console.warn === 'function') {
                  console.warn('[Commentaires] Publication SharePoint impossible :', error);
                }
              });
          });
        };
  ...
  const handleAddSharedMember = useCallback((email) => {
    ...
    projectMembersProvider.addMember(activeProjectId, normalizedEmail).catch((error) => {
      if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn('[Membres] Ajout SharePoint impossible :', error);
      }
    });
  ...
  const handleRemoveSharedMember = useCallback((email) => {
    ...
    projectMembersProvider.removeMember(activeProjectId, normalizedEmail).catch((error) => {
      if (typeof console !== 'undefined' && typeof console.warn === 'function') {
        console.warn('[Membres] Suppression SharePoint impossible :', error);
      }
    });
  }, [activeProjectId]);
  ...
  const handleCreateNewProject = useCallback(() => {
    ...
    const projectName = inferredName && inferredName.trim().length > 0 ? inferredName.trim() : 'Projet sans nom';
  ...
  }, [handleSaveProject, notifyProjectSubmission, unansweredMandatoryQuestions]);
  ...
  const handleSharePointReinitialization = useCallback(async () => {
    setSharePointReinitState({
      inProgress: true,
      message: 'Vérification de la structure SharePoint…',
      status: 'pending'
    });
    ...
    try {
      ...
      if (alreadyPublished.length > 0) {
        const confirmed =
          typeof window === 'undefined' || typeof window.confirm !== 'function'
            ? true
            : window.confirm(
              `Une configuration est déjà publiée (${alreadyPublished.length} fichier(s)).\n\n` +
                  'La remplacer écrasera la configuration visible par tous les utilisateurs. Continuer ?'
            );

        if (!confirmed) {
          setSharePointReinitState({
            inProgress: false,
            message: 'Publication annulée : la configuration publiée est inchangée.',
            status: 'idle'
          });
          return;
        }
      ...
      setSharePointReinitState({
        inProgress: true,
        message: 'Publication de la configuration vers SharePoint…',
        status: 'pending'
      });
      ...
      setSharePointReinitState({
        inProgress: false,
        message: `Configuration publiée dans ${summary.libraryName} (${details}).`,
        status: 'success'
      });
    } catch (error) {
      setSharePointReinitState({
  ...
  return (
    <div className={`min-h-screen ${annotationOffsetClass}`}>
      {sharePointSync.state === 'loading' && (
        ...
        >
          <div className="mx-auto max-w-2xl">
            Synchronisation avec SharePoint en cours…
          </div>
        </div>
      )}
      ...
      {sharePointSync.state === 'partial' && (
        ...
        >
          <div className="mx-auto max-w-2xl">
            Connecté à SharePoint, avec une réserve : {sharePointSync.message}. Un administrateur
            peut publier la configuration depuis le back-office.
          </div>
        </div>
      )}
      ...
      {sharePointSync.state === 'error' && (
        ...
        >
          <div className="mx-auto max-w-2xl">
            Impossible de charger les données depuis SharePoint ({sharePointSync.message}). Vous
            pouvez continuer à travailler : vos modifications sont conservées sur cet ordinateur et
            seront envoyées à la prochaine connexion réussie.
          </div>
        </div>
      )}
      ...
      {sharePointSync.state === 'session-expired' && (
        ...
        >
          <div className="mx-auto max-w-2xl">
            Votre session SharePoint a expiré.{' '}
            <button
              type="button"
      ...
      {persistenceError && (
        ...
        >
          <div className="mx-auto flex max-w-2xl items-start justify-between gap-3">
            <span>
              La sauvegarde locale a échoué (espace de stockage du navigateur saturé). Vos dernières
              modifications risquent de ne pas être conservées. Exportez votre projet pour ne rien
              perdre, puis libérez de l’espace.
            </span>
            <button
              type="button"
                  ...
                  <button
                    ...
                    aria-label={
                      tourGuideStatus === 'loading'
                        ? 'Guide interactif en cours de préparation'
                        : tourGuideStatus === 'error'
                          ? 'Guide interactif indisponible'
                          : 'Lancer le guide interactif'
                    }
                    title={
                      tourGuideStatus === 'error'
                        ? 'Le guide n’a pas pu être chargé. Rechargez la page pour réessayer.'
                        : undefined
                    }
                  >
                    {tourGuideStatus === 'loading' ? (
